...
let template__card = await loadTemplate('./component/Team/template__card.html');
...
Team.format =  function(obj) {
    ...
    html = html.replace('{{bouton}}', obj.bouton);
    html = html.replace('{{background}}', obj.background);

    let liste = []
    for (let card in obj.cards){
        let html2 = template__card;
        html2 = html2.replace('{{img--profil}}', obj.cards[card].imgProfil);
        html2 = html2.replace('{{name--profil}}', obj.cards[card].nameProfil);
        html2 = html2.replace('{{svg--icon}}', obj.cards[card].svgIcon);
        liste += html2;
    }
...
Team.render = async function(selector, jsonFilename){
    ...
    let html = '';
    for(let obj of data){
        html += Team.format(obj); // on concatène les HTML des composants formatés  
    }
