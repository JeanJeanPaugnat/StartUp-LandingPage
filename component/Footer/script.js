...
let template__card = await loadTemplate('./component/Footer/template__card.html');
...
Footer.format =  function(obj) {
    let html = template;
    html = html.replace('{{footer__credits}}', obj.credit);

    let liste = []
    for (let card in obj.cards){
        let html2 = template__card;
        html2 = html2.replace('{{section__name}}', obj.cards[card].nameSection);
        liste += html2;
    }
...
Footer.render = async function(selector, jsonFilename){
    ...
    let html = '';
    for(let obj of data){
        html += Footer.format(obj); // on concatène les HTML des composants formatés  
    }
