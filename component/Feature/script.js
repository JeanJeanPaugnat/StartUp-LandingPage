...
let template = await loadTemplate('./component/Feature/template.html');
let template__card = await loadTemplate('./component/Feature/template__card.html');
...
Feature.format =  function(obj) {
    ...
    for (let card in obj.cards){
        ...
        html2 = html2.replace('{{svg__icon}}', obj.cards[card].svgIcon);
        html2 = html2.replace('{{title__card}}', obj.cards[card].titleCard);
        html2 = html2.replace('{{text__card}}', obj.cards[card].textCard);
        liste += html2;
    }
...
Feature.render = async function(selector, jsonFilename){
    ...
    let html = '';
    for(let obj of data){
        html += Feature.format(obj); // on concatène les HTML des composants formatés  
    }
